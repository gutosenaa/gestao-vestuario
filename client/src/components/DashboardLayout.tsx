...
export default function DashboardLayout({
  ...
  return <div className="cinema-bg min-h-screen text-slate-100">
    ...
    {mobileOpen && <div className="fixed inset-0 z-[60] lg:hidden"><button aria-label="Fechar menu" onClick={() => setMobileOpen(false)} className="absolute inset-0 bg-slate-950/70" /><aside className="absolute inset-y-0 left-0 w-[290px] border-r border-white/10 bg-slate-950 p-4 shadow-2xl"><div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2 text-cyan-200"><Store size={21} /><span className="font-display text-sm font-bold tracking-widest">FEIRENSE</span></div><button onClick={() => setMobileOpen(false)} className="rounded-lg p-2 text-slate-400"><X size={20} /></button></div><div className="space-y-1">{visibleNavigation.map(item => { const Icon = item.icon; return <button key={item.id} onClick={() => navigate(item.id)} className={`flex h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm ${activeView === item.id ? "bg-cyan-300/15 text-cyan-100" : "text-slate-400"}`}><Icon size={18} />{item.label}</button>; })}</div><div className="absolute bottom-5 left-4 right-4"><div className="rounded-xl bg-white/5 p-3 text-xs text-slate-300"><p className="font-medium text-white">{user.name || "Usuário"}</p><p className="mt-1 text-cyan-300">{user.role}</p><button onClick={logout} className="mt-3 flex items-center gap-2 text-orange-200"><LogOut size={14} />Sair</button></div></div></aside></div>}
  </div>;
}
