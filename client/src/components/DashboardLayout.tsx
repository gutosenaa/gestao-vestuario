...
export default function DashboardLayout({
  ...
  const openAlerts = () => { navigate("dashboard"); window.setTimeout(() => { const target = document.getElementById("alerts-panel"); target?.scrollIntoView({ behavior: "smooth", block: "center" }); target?.focus({ preventScroll: true }); }, 80); };
  return <div className="cinema-bg min-h-screen text-slate-100">
    <aside className={`fixed inset-y-0 left-0 z-50 hidden border-r border-white/8 bg-slate-950/65 backdrop-blur-2xl transition-all duration-200 lg:flex lg:flex-col ${collapsed ? "w-[76px]" : "w-[248px]"}`}>
      <div className="flex h-[76px] items-center gap-3 border-b border-white/8 px-5">
        ...
        {!collapsed && <div className="min-w-0"><p className="font-display text-sm font-bold tracking-[.17em] text-white">FEIRENSE</p><p className="text-[10px] tracking-[.22em] text-cyan-300">GESTÃO ESPORTIVA</p></div>}
      </div>
      <nav className="flex-1 space-y-1 overflow-y-auto p-3 pt-5">
        {visibleNavigation.map(item => { const Icon = item.icon; const selected = item.id === activeView; return <button aria-current={selected ? "page" : undefined} aria-label={item.label} key={item.id} onClick={() => navigate(item.id)} className={`group flex h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm transition ${selected ? "bg-gradient-to-r from-cyan-300/18 to-teal-400/8 text-white shadow-[inset_0_0_0_1px_rgba(103,232,249,.12)]" : "text-slate-400 hover:bg-white/6 hover:text-slate-100"}`} title={collapsed ? item.label : undefined}><Icon size={18} className={selected ? "text-cyan-300" : "text-slate-500 group-hover:text-slate-300"} />{!collapsed && <span className="font-medium">{item.label}</span>}{item.quick && !collapsed && <span className="ml-auto rounded-md bg-orange-300/12 px-1.5 py-0.5 text-[9px] font-bold tracking-wider text-orange-200">RÁPIDA</span>}</button>; })}
      </nav>
      <div className="border-t border-white/8 p-3">
        <button aria-label={collapsed ? "Expandir menu" : "Recolher menu"} aria-expanded={!collapsed} onClick={() => setCollapsed(value => !value)} className="mb-3 flex h-10 w-full items-center justify-center rounded-xl text-slate-400 hover:bg-white/6 hover:text-white">{collapsed ? <ChevronRight size={18} /> : <><ChevronLeft size={17} /><span className="ml-2 text-xs">Recolher menu</span></>}</button>
    ...
    <header className="sticky top-0 z-30 flex h-[68px] items-center gap-3 border-b border-white/8 bg-slate-950/45 px-4 backdrop-blur-xl lg:ml-[248px] lg:px-7">
      <button aria-label="Abrir menu de navegação" onClick={() => setMobileOpen(true)} className="grid size-9 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-100 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-200 lg:hidden"><Menu size={19} /></button>
      ...
      <div className="relative hidden max-w-xs flex-1 md:block"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" /><input aria-label="Busca global" onFocus={() => setSearchOpen(true)} onBlur={() => setSearchOpen(false)} onChange={event => onGlobalSearch?.(event.target.value)} placeholder="Buscar produto, cliente, código..." className="h-10 w-full rounded-xl border border-white/8 bg-white/[.045] pl-9 pr-3 text-xs text-white outline-none transition placeholder:text-slate-500 focus:border-cyan-300/40 focus:bg-white/[.075]" />{searchOpen && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[9px] text-slate-500">BUSCA GLOBAL</span>}{searchOpen && globalSearchResults.length > 0 && <div role="listbox" aria-label="Resultados da busca global" className="absolute left-0 right-0 top-12 z-50 max-h-[340px] overflow-y-auto rounded-xl border border-white/10 bg-slate-950/95 p-1.5 shadow-2xl backdrop-blur-xl">{globalSearchResults.map(result => <button role="option" aria-label={`${result.type}: ${result.label}`} key={`${result.type}-${result.id}`} onMouseDown={event => event.preventDefault()} onClick={() => { onGlobalResultSelect?.(result); setSearchOpen(false); }} className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-white/6 focus-visible:outline-2 focus-visible:outline-cyan-200"><div className="min-w-0"><p className="truncate text-xs font-medium text-white">{result.label}</p><p className="mt-0.5 truncate text-[10px] text-slate-500">{result.subtitle}</p></div><span className="shrink-0 text-[9px] font-bold tracking-wider text-cyan-300">{result.type.toUpperCase()}</span></button>)}</div>}</div>
      <button aria-label={`Abrir alertas${alertCount ? `, ${alertCount} pendente(s)` : ""}`} onClick={openAlerts} className="relative grid size-10 place-items-center rounded-xl border border-white/8 bg-white/[.045] text-slate-300 hover:text-cyan-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-200"><Bell size={17} />{alertCount > 0 && <span aria-hidden="true" className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full bg-orange-400 text-[9px] font-bold text-slate-950">{Math.min(alertCount, 9)}</span>}</button>
    ...
    <nav aria-label="Navegação principal móvel" className="fixed bottom-0 z-40 flex h-[72px] w-full items-center justify-around border-t border-white/10 bg-slate-950/85 px-2 pb-safe backdrop-blur-xl lg:hidden">{visibleNavigation.filter(item => ["dashboard", "venda", "produtos", "estoque", "financeiro"].includes(item.id)).slice(0, 5).map(item => { const Icon = item.icon; const selected = activeView === item.id; return <button aria-current={selected ? "page" : undefined} key={item.id} onClick={() => navigate(item.id)} className={`flex min-w-[52px] flex-col items-center gap-1 text-[9px] font-medium focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-200 ${selected ? "text-cyan-200" : "text-slate-500"}`}><span className={`grid size-8 place-items-center rounded-xl ${selected ? "bg-cyan-300/15" : ""}`}><Icon size={17} /></span>{item.label === "Visão geral" ? "Início" : item.label}</button>; })}</nav>
    {mobileOpen && <div className="fixed inset-0 z-[60] lg:hidden"><button aria-label="Fechar menu" onClick={() => setMobileOpen(false)} className="absolute inset-0 bg-slate-950/70" /><aside aria-label="Menu de navegação móvel" className="absolute inset-y-0 left-0 w-[290px] border-r border-white/10 bg-slate-950 p-4 shadow-2xl"><div className="mb-6 flex items-center justify-between"><div className="flex items-center gap-2 text-cyan-200"><Store size={21} /><span className="font-display text-sm font-bold tracking-widest">FEIRENSE</span></div><button aria-label="Fechar menu" onClick={() => setMobileOpen(false)} className="rounded-lg p-2 text-slate-400 focus-visible:outline-2 focus-visible:outline-cyan-200"><X size={20} /></button></div><nav aria-label="Módulos do sistema" className="space-y-1">{visibleNavigation.map(item => { const Icon = item.icon; return <button aria-current={activeView === item.id ? "page" : undefined} key={item.id} onClick={() => navigate(item.id)} className={`flex h-11 w-full items-center gap-3 rounded-xl px-3 text-left text-sm focus-visible:outline-2 focus-visible:outline-cyan-200 ${activeView === item.id ? "bg-cyan-300/15 text-cyan-100" : "text-slate-400"}`}><Icon size={18} />{item.label}</button>; })}</nav><div className="absolute bottom-5 left-4 right-4"><div className="rounded-xl bg-white/5 p-3 text-xs text-slate-300"><p className="font-medium text-white">{user.name || "Usuário"}</p><p className="mt-1 text-cyan-300">{user.role}</p><button aria-label="Sair" onClick={logout} className="mt-3 flex items-center gap-2 text-orange-200 focus-visible:outline-2 focus-visible:outline-cyan-200"><LogOut size={14} />Sair</button></div></div></aside></div>}
  </div>;
}
